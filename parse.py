...
class Parse:
    ...
    def __init__(self):
        ...
        start = (L("<").suppress()                                                             \
                 + Parse.start_strings                                                         \
                 + Optional( White().suppress() + Combine(OneOrMore(~L(">") + Parse.charset))).suppress() \
                 + L(">").suppress()) # Assumes no escaped '>'
        end = (L("</")                                                                         \
               + Parse.start_strings                                                          \
               + Optional( White() + Combine(OneOrMore(~L(">") + Parse.charset))) \
               + L(">")).suppress()
        entity = Regex("<br>|&.*?;").setParseAction( lambda toks: self.entity_table[toks[0]])
        string = Combine(OneOrMore( Combine(OneOrMore( ~(end | start | entity) + Parse.charset)) | entity))
